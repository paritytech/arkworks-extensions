    ...
    fn multi_miller_loop(
        ...
        let a: Vec<Vec<u8>> = a
            ...
            .map(|elem| {
                ...
                <<Bls12<Self> as Pairing>::G1Prepared as CanonicalSerialize>::serialize_uncompressed(&serialized_result[..]);
                serialized_result
            })
            .collect();
        let b = b
            ...
            .map(|elem| {
                ...
                let mut serialized_result = vec![0u8; elem.serialized_size(Compress::No)];
                let mut cursor = Cursor::new(&mut serialized_result[..]);
                <<Bls12<Self> as Pairing>::G2Prepared as CanonicalSerialize>::serialize_uncompressed(&serialized_result[..]);
