...
use ark_ec::{scalar_mul::variable_base::VariableBaseMSM, AffineRepr, Group};
...
pub trait SWCurveConfig: ark_ec::CurveConfig {
	...
	fn serialize_with_mode<W: Write>(
		...
		let (x, y, flags) = match item.infinity {
			true => (Self::BaseField::zero(), Self::BaseField::zero(), SWFlags::infinity()),
			...
			Compress::Yes => {
				...
				match flags {
					SWFlags::PointAtInfinity =>
						(Affine::<Self>::identity().x, Affine::<Self>::identity().y, flags),
					_ => {
